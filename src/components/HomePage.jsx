      ...
      {groceries.categories.map((category) => (
        <div key={category.category}>
          <div className="grocery-type">
            <img
              src={categoryImages[`${category.category.toLowerCase()}`]}
            ...
            {category.items.map((item) => (
              <div className="item-card" key={item.name}>
                <img
                  ...
                  alt={item.name} className="item-pic"
                />
                ...
                <div className="quantity">
                  ...
                  <input
                    type="text"
                    value={items[item.name]?.qty || 0}
                    onChange={(e) => changeHandler(e, item.name)}
                  />
                  <button onClick={() => increment(item.name)}>+</button>
                </div>
                <button onClick={(e) => addToCart(e, item.name)}>Add to
